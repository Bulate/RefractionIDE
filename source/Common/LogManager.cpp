...
ResultsDockWidget* LogManager::resultsDockWidget = nullptr;
...
//  #endif
    ...
    if ( resultsDockWidget && shouldLogToGui(type, category) )
        resultsDockWidget->appendDebuggerMessage(type, category, reducedMessage);
...
QString LogManager::buildLogFilename(bool saveInSettings)
{
    ...
    const QString& filename = QString("%1/log--%3.csv").arg(dirPath)/*.arg(hostname)*/.arg(datetime);
